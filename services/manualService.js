...
class ManualService {
  ...
  async analyzeDocument(content, existingTags, provider) {
    try {
      ...
      console.error('Error analyzing document: Invalid provider');
    } catch (error) {
  ...
  async _analyzeOpenAI(content, existingTags) {
    try {
      ...
      let jsonContent = extractChatMessageContent(
        response?.choices?.[0]?.message,
        'ManualService/OpenAI'
      );
      if (!jsonContent) {
  ...
  async _analyzeOllama(content, existingTags) {
    try {
      ...
      const ollamaApiKey =
        process.env.OLLAMA_API_KEY || config.ollama.apiKey || '';
      if (ollamaApiKey) {
